...
rf_model <- randomForest(selling_price ~ ., data=pred_data, na.action = na.omit)

# Viewing the model summary
print(rf_model)
# Plotting variable importance
varImpPlot(rf_model)
# Predicting selling prices using the Random Forest model
predictions <- predict(rf_model, newdata=pred_data)

# You can compare these predictions with actual values
# Here, just showing the first few predictions
head(predictions)
...
# Assuming your data is already preprocessed and named as pred_data
# Ensure that the target variable is numeric
pred_data$selling_price <- as.numeric(pred_data$selling_price)

# Split data into training and testing sets
set.seed(123)  # for reproducibility
training_indices <- sample(1:nrow(pred_data), 0.8 * nrow(pred_data))
train_data <- pred_data[training_indices, ]
test_data <- pred_data[-training_indices, ]

# Prepare the data for glmnet
x_train <- as.matrix(train_data[, -which(names(train_data) == "selling_price")])
y_train <- train_data$selling_price

x_test <- as.matrix(test_data[, -which(names(test_data) == "selling_price")])
...
# Use the best lambda for prediction
